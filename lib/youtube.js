...
var Youtube = (function(){
    ...
    for (key in videoData) {
      ...
      if(typeof video !== 'undefined') {
        ...
        if(typeof video.id !== 'undefined') {
          ...
          else if(typeof(video.id) === 'object') {
          ...
          // Get the URL
          result.url = 'http://youtube.com/watch?v=' + result.id;
        }
        else continue;
  ...
  youtube.prototype.query = function(query, callback){
    ...
    request(httpOpts, function(response){
      ...
      if(response !== null) {
        ...
        _this.getVideoDurations(cleanData, callback);
      }
      else {
  ...
  youtube.prototype.getVideoDurations = function(videoData, callback){

    var _this = this;
    var videoIdsStr = '';
    for(videoItem in videoData) {

      videoIdsStr += videoData[videoItem].id + ',';
    }
    ...
    request(httpOpts, function(response){
      ...
      if(response !== null) {
        ...
        for(var i = 0; i < response.items.length; i++) {
          ...
          var duration = response.items[i].contentDetails.duration;
          videoData[i].duration = _this.convertDurationToSeconds(duration);
        }
        callback(videoData);
      }
      else {
